...
static int try_load_corr_dupl(char **row, int count, t_globals *g, t_corr_state *c, char *needle)
{
	...
	if (c->stage == 1) 
	{
		...
		if (c->col >= count) 
		{
			fprintf(stderr, "Error: %s alpha missing \n", needle);
			return -1;
		}

		if (parse_long_double(row[c->col], &g->alpha, "alpha") != 0)
			return -1;
...
while (1) 
{
	...
	if (err != PARSE_OK) 
	{
		fprintf(stderr,"Error: parse failure before header\n");
		p->close(p);
		return -1;
	}
	// Try load Universo 
	int r = try_load_universe(header, hcount, g,  fields[6]);
	if (r < 0)
		goto fail;
	int b = try_load_universe(header, hcount, g,  "Presupuesto");
	if(b < 0 )
		goto fail;
	// Try load Corr. Dupl //
	if (try_load_corr_dupl(header, hcount, g, &corr,  fields[5]) < 0)
		goto fail;
...
//printf("rows found\n");
	...
	while (1) 
	{
		...
		if (!node->name) 
		{
			fprintf(stderr, "Error: out of memory at (column '%s')\n",fields[map->name]);
			free(node);
			goto fail;
		}
		if(node->name[0] == '\0')
